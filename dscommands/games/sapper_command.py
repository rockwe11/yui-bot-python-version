...
class SapperCommand:
    ...
    async def handle(self, ctx: CommandContext):
        ...
        if len(ctx.get_args()) > 0 and not self.get_player(player_id):
            ...
            if not ctx.get_args()[0].isdigit() or not ctx.get_args()[1].isdigit():
                await ctx.send_message("❌ Аргументы должны быть целочисленными числами")
    ...
    def get_help(self):
        return "🧩 Игра « Сапёр »\n\n" \
               "{prefix}sapper [Ставка] - начать игру\n" \
               "{prefix}sapper [X] [Y] - открыть клетку\n" \
               "{prefix}sapper fl [X] [Y] - установить флажок\n\n" \
